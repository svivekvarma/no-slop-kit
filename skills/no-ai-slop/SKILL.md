---
name: no-slop
description: Edit drafts into sharper, more human writing while preserving the writer's personal voice, or detect AI-slop patterns without rewriting. Applies channel-specific rules for technical docs, articles, blogs, emails, chat messages, social posts, social comments, code comments, commit messages and PR bodies. Use when the user wants a draft clearer, more direct, more opinionated or less AI-sounding, asks whether writing reads as AI, or is about to publish prose to other people.
slop-lint: off
---

<!--
Provenance: the Editing principles, Words to cut and Patterns to cut sections
below are reproduced from petergyang/no-ai-slop (MIT, Copyright (c) 2026 Peter
Yang) with light edits. The Channel router, Channels table and the workflow
steps that reference the linter are new to no-slop-kit. See NOTICE.
-->
...
Slop is channel-specific. A 900-word structure with headings is right for a runbook and wrong for a Slack message. Before editing, decide which channel the draft belongs to and read that file:

| Channel | Read | Covers |
| --- | --- | --- |
| `technical-docs` | `channels/technical-docs.md` | READMEs, API docs, runbooks, ADRs, release notes |
| `articles-and-blogs` | `channels/articles-and-blogs.md` | Long-form articles, blog posts, newsletters |
| `email` | `channels/email.md` | Work email, outreach, replies |
| `messages` | `channels/messages.md` | Slack, Teams, Discord, DMs, SMS |
| `social-posts` | `channels/social-posts.md` | LinkedIn, X, Threads, Bluesky posts |
| `social-comments` | `channels/social-comments.md` | Replies and comments on social posts |
| `code-comments` | `channels/code-comments.md` | Inline comments, docstrings, TODOs |
| `pull-requests` | `channels/pull-requests.md` | Commit messages, PR titles and bodies |
| `review-comments` | `channels/review-comments.md` | Code review comments and PR replies |

Read exactly one channel file, the one that matches. Each is short and holds only what differs from the shared rules below. If the channel is genuinely unclear, ask one question: who is this for and where will it be published? If the user names no channel and the draft is a file in the repo, infer from the path the way `scripts/slop_lint.py` does.

## Running the linter

`scripts/slop_lint.py` is a regex linter that costs no model tokens. It finds the mechanical patterns fast and misses everything that needs judgement. Use it as a first pass, never as the verdict.

```sh
python scripts/slop_lint.py DRAFT.md --channel technical-docs
python scripts/slop_lint.py DRAFT.md --format json --limit 10
```

It reports three severities. `slop` is a signature pattern and is almost always worth fixing. `likely` is usually worth fixing. `check` needs your judgement in context and is often correct as written. A finding is a prompt to look, not an instruction to change. Never edit a line only to clear a linter hit, and never report a score as if it measured writing quality.

What the linter cannot see: whether a claim is true, whether a detail is the useful one, whether cut text carried the writer's voice, whether the structure serves the reader, and whether synonym cycling is deliberate. That is your job.
...
Often-empty phrases: it's worth noting, it's important to note, at the end of the day, when it comes to, at its core, in today's world, in the age of, in the world of, the reality is, the truth is, in terms of, with regard to, in order to, going forward, in this article, let's dive in, needless to say, first and foremost, last but not least. Cut them when they delay the point. Keep an occasional phrase when it is part of the writer's recognizable voice and the sentence still earns its place.
...
2. Decide the channel and read that one file from `channels/`.
...
4. Optionally run `scripts/slop_lint.py` for a free first pass on the mechanical patterns.
5. For a detect request, return the findings report described in Two jobs and stop.
6. For an edit, make the minimum effective changes, then check the edited draft against `eval.md` yourself, including the channel section.
